...
*調整後。1〜2文にまとまり、雨の確率が高いので傘を勧めている。ただし降水確率 80% は今日（9月22日）の値で、「明日」と取り違えている（「現時点の制約」を参照）。*
...
**天気予報の「今日」と「明日」を取り違えることがある。**
ツールは予報を日付（`2026-09-22` など）だけで返しており、モデルは天気の質問で今日の日付を確認しないため、
1日目の予報を「明日」として読むことがある。日ごとに「今日」「明日」のラベルを付けて渡せば解消できる見込み。
