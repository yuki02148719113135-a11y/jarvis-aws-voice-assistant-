...
### ツールの戻り値が大きいと Invalid input request で切れる

天気を聞くと `session ended with error: Invalid input request` でセッションが落ちた。
日時ツールでは起きず、天気だけで起きる。予報を3日分返していたのを2日分に減らしたら通った。
読み上げに使うのは今日と明日なので、実用上の支障はない。
ツールの戻り値には上限があると考えられるが、エラーメッセージからは分からなかった。
...
**履歴に残った失敗した返答を、モデルが手本にしてしまう。**
履歴を書き起こしとして渡しているため、過去にうまく答えられなかったやり取りが残っていると、
同じ調子の返答を繰り返すことがある。実際、ツール連携が未完成だった頃の履歴が残った状態では、
名前を聞いても「覚えることはできません」と答えた。履歴を消すと正しく答える。
