...
会話の状態を Nova Sonic のセッション（＝タスクのメモリ）から DynamoDB へ移した。
どのタスクに繋がっても続きから話せるようになり、ALB のスティッキーセッションが不要になった。

| 項目 | 内容 |
|---|---|
| テーブル | `jarvis-conversations`（`session_id` + `created_at`、`expires_at` で TTL 7日） |
| セッションID | ブラウザの `localStorage` が持ち、WebSocket 接続時にクエリで渡す |
| 保存する内容 | 確定した発言のテキストのみ（音声は保存しない） |
| 復元する量 | 直近 20 件 |

![復元された会話](docs/sprint3-restored-browser.png)

*ブラウザを閉じて開き直しても、「前回の続き（N件を復元）」と表示され、前に伝えた名前に答えられる。*

![復元時のログ](docs/sprint3-restored-log.png)

*同じときのサーバーログ。`session=...` に対して 15 件の履歴を読み込んでいる。*

![複数タスクでの継続](docs/sprint3-multi-task.png)

*AWS 上でタスクを2つ動かし、接続し直したときのログ。3回目は行頭のタスクIDが
`4a3b61e9...` から `418b5794...` に変わっているが、同じ `session=` で履歴が
4件まで積み上がっている。別タスクに振られても会話が続く。*

### 設計上の判断

**DynamoDB の読み書きは `memory.py` に閉じ込める。**
`main.py` は「接続時に読む」「発言が確定したら書く」しか知らない。
読み書きは `asyncio.to_thread` に逃がし、音声のやり取りを止めない。
失敗しても例外は投げず、履歴なしの新規会話として続行する。

**履歴はシステムプロンプトに渡す。ただしユーザーの発言だけ。**
`interactive: false` の TEXT コンテンツとして流し込む方法を先に試したが、
モデルの文脈には入らなかった（「詰まった点」を参照）。

当初は両者の発言を書き起こして渡していたが、過去にうまく答えられなかったやり取りが
残っていると、モデルがそれを手本にして同じ調子で答えた。ツール連携が未完成だった頃の
履歴が残った状態では、名前を聞いても「覚えることはできません」と返した。
覚えておきたいのはユーザーが何を言ったかなので、自分の発言は渡さないことにした。
プロンプトが短くなる副次的な効果もあった。

**音声は保存しない。**
必要なのは文脈であって録音ではない。テキストだけなら1発言あたり数百バイトで、
TTL で7日後に自動的に消える。

**セッションIDはブラウザが持つ。**
サーバー側で発行して Cookie に入れる方法もあるが、
`localStorage` なら「履歴を消す」操作をブラウザ側だけで完結できる。
...
![ログイン画面](docs/sprint4-cognito-login.png)

*Cognito の Hosted UI。ログイン画面は自前で作っていない。*

ユーザープールは persistent 側に置いた。ephemeral に置くと、消すたびにユーザーが
消えて登録し直しになる。月5万アクティブユーザーまで無料。

**ALB の認証機能ではなくアプリ側で検証した。**
ALB には `authenticate-cognito` アクションがあり、設定だけで認証を挟める。
ただしリダイレクト先に正式な証明書が要り、自己署名で検証している今の構成では使えない。
Hosted UI でログインさせ、受け取った ID トークンを WebSocket 接続時に渡して
サーバーで検証する形にした（`auth.py`）。署名・有効期限・発行者・宛先を確認している。

**履歴のキーをログインしたユーザーの ID にした。**
Sprint 3 ではブラウザの `localStorage` が持つ ID を使っていたため、端末を変えると
続きから話せなかった。ログイン後は Cognito の `sub` をキーにするので、
別の端末やブラウザからでも同じ履歴を引き継げる。

**環境変数が未設定なら認証を飛ばす。**
ローカル開発でログインを挟むと確認が面倒になるため、
`COGNITO_USER_POOL_ID` が空のときは検証しない。
...
### 5. スティッキーセッションを有効にし、Sprint 3 で無効化した

Nova Sonic のセッションはタスクのメモリ上にあるため、
再接続時に別タスクへ振られると会話が失われる。Sprint 2 まではこれが唯一の会話継続手段だった。
Sprint 3 で履歴を DynamoDB へ外出ししたことで不要になり、`enabled = false` にしている。
下の画像は有効だった当時のもの。
...
### 履歴を interactive: false の TEXT コンテンツで渡しても文脈に入らない

会話履歴を復元する方法として、`promptStart` の後・音声コンテンツを開く前に、
role を USER / ASSISTANT にした TEXT コンテンツ（`interactive: false`）を
発言の数だけ流し込む形を試した。エラーは出ないが、モデルは履歴を参照せず
「前回の会話の情報は保持していません」と答えた。
システムプロンプトの末尾に `User:` / `You:` の書き起こしとして混ぜたところ、
正しく前回の内容を答えるようになった。

### Fargate ではタスクロールを読むパッケージが別に要る

デプロイすると、接続はできるのに返事が返らなかった。ログに1行だけ出ていた警告が原因。

```
EcsContainer credential source was detected but no provider claims it;
install 'aws-credentials-http'
```

ローカルは `~/.aws` をマウントしていたので動いていたが、Fargate ではタスクロールから
認証情報を取る必要があり、Bedrock 用 SDK はそのための実装を別パッケージに分けている。
`requirements.txt` に `aws-credentials-http` を足して解決した。
エラーではなく警告として出るため、見落としやすい。

### ID トークンの at_hash 検証でつまずく

`python-jose` で ID トークンを検証すると
`No access_token provided to compare against at_hash claim` で弾かれた。
ID トークンにはアクセストークンの指紋（`at_hash`）が入っており、突き合わせる相手を
渡していないためだった。今回はアクセストークンを送らない設計なので、
`options={"verify_at_hash": False}` で無効化した。署名・有効期限・発行者・宛先の検証は残る。

### Invalid input request の原因はツールではなくシステムプロンプトだった

天気を聞くと `session ended with error: Invalid input request` でセッションが落ちた。
日時ツールでは起きず、天気だけで落ちるため、戻り値の大きさを疑って予報を3日分から
2日分に減らしたところ通った。ここで打ち切っていれば、原因を取り違えたままだった。

実際の原因は履歴の渡し方だった。当時はユーザーとアシスタント両方の発言を
書き起こしてシステムプロンプトに入れており、18件も溜まるとかなりの長さになる。
そこへ3日分の予報が加わって上限に触れていた。
履歴をユーザーの発言だけに絞ったところ、3日分でも落ちなくなった。

症状が出た場所（天気ツール）と原因の場所（履歴）が離れていると、
目の前の変数を動かして通ったところで止めてしまいやすい。
...
**履歴の復元はセッションIDを持つブラウザに限られる。**
`localStorage` を消すか別の端末から開くと、新しい会話として始まる。
利用者と履歴を結びつけるには Sprint 4 の認証が必要。

**漢字の読みを間違えることがある。**
「霧雨」を別の読みで発音した。指摘すると読み方自体は正しく答えるので、
知識ではなく音声合成側の問題。日本語が公式には未対応であることの実例。
...
- **Sprint 3** — 完了。会話履歴を DynamoDB へ永続化し、スティッキーセッションを無効化。
  AWS 上で2タスクを動かし、別タスクへ振られても会話が続くことを確認済み
- **Sprint 4** — 完了。CI/CD（ビルドと ECR への push の自動化）と Cognito 認証
- **次** — Cognito の環境変数を ECS タスク定義へ渡す（現状 AWS 上では認証が無効）、
  独自ドメインと ACM による正式な HTTPS 化
